...
func NewRaftStorage(isClean bool) (*RaftStorage, error) {
	if isClean {
		return NewRaftStorageWithClean()
	}
...
func NewRaftStorageWithClean() (*RaftStorage, error) {
	rs := &RaftStorage{}
...
func (rs *RaftStorage) MarshalKey(idx uint64) ([]byte, error) {
	...
	copy(theBytes[pttdb.SizeDBKeyPrefix:], MyID[:])
