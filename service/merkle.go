...
func NewMerkle(dbOplogPrefix []byte, dbMerklePrefix []byte, prefixID *types.PttID, db *pttdb.LDBBatch, entity Entity) (*Merkle, error) {
	...
	if err == leveldb.ErrNotFound {
		lastGenerateTS = entity.GetCreateTS()
	}
...
func (m *Merkle) SaveMerkleTreeCore(level MerkleTreeLevel, ts types.Timestamp, nextTS types.Timestamp, updateTS types.Timestamp) (types.Timestamp, error) {
	// 1. get iter
	childLevel := level - 1
	iter, err := m.GetMerkleIter(childLevel, ts, nextTS)
	if err != nil {
...
func (m *Merkle) GetMerkleTreeListCore(level MerkleTreeLevel, ts types.Timestamp, nextTS types.Timestamp) ([]*MerkleNode, error) {
	...
	iter, err := m.GetMerkleIter(level, ts, nextTS)
	if err != nil {
		return nil, err
	}
	defer iter.Release()
...
func (m *Merkle) GetMerkleIter(level MerkleTreeLevel, ts types.Timestamp, nextTS types.Timestamp) (iterator.Iterator, error) {
	...
	theRange := &util.Range{Start: startKey, Limit: endKey}

	//log.Debug("GetMerkleIter", "start", startKey, "limit", endKey)

	return m.db.DB().NewIteratorWithRange(theRange), nil
}

func (m *Merkle) GetMerkleIterByKey(startKey []byte, level MerkleTreeLevel) (iterator.Iterator, error) {
	prefix := append(m.DBPrefix(), byte(level))

	//log.Debug("GetMerkleIterByKey", "prefix", prefix, "startKey", startKey)

	return m.db.DB().NewIteratorWithPrefix(startKey, prefix)
}
